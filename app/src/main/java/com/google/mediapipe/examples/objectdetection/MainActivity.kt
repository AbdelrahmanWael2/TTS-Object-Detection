...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        downloadButton.setOnClickListener {
            ...
            val cameraFragment = navHostFragment.childFragmentManager.fragments.firstOrNull { it is CameraFragment } as? CameraFragment

            cameraFragment?.downloadSpokenWords()        }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == WRITE_EXTERNAL_STORAGE_REQUEST_CODE) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                // Permission granted, you can now call the downloadSpokenWords function
            } else {
